...
ATTR_REQUEST volatile struct limine_memmap_request memmap_request = {.id = LIMINE_MEMMAP_REQUEST, .revision = 0};
ATTR_REQUEST volatile struct limine_hhdm_request hhdm_request = {.id = LIMINE_HHDM_REQUEST, .revision = 0};
ATTR_REQUEST volatile struct limine_kernel_address_request kernel_address_request = {.id = LIMINE_KERNEL_ADDRESS_REQUEST, .revision = 0};
...
void init_memory(void)
{
	...
	// will ensure if malloc misbehaves and we overwrite we will get a page fault.
	uintptr_t heap_virtual_address = (uintptr_t)&kernel_end;
	if (heap_virtual_address % PAGE_BYTE_SIZE)
	{
		heap_virtual_address += (PAGE_BYTE_SIZE - (heap_virtual_address % PAGE_BYTE_SIZE));
	}
