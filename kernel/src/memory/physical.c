...
static inline size_t page_align_size(size_t size_in_bytes)
{
	...
	{
		return size_in_bytes + (PAGE_BYTE_SIZE - (size_in_bytes % PAGE_BYTE_SIZE));
	}
...
{
	return (size_in_bytes / PAGE_BYTE_SIZE) + (size_in_bytes % PAGE_BYTE_SIZE ? 1 : 0);
}

// Determines if the page is used.
static inline bool is_page_used(struct PHYSICAL_MEMORY_CONTEXT *memory, uintptr_t page_index)
{
	return memory->bitmap[page_index / PAGES_PER_BITMAP_INDEX] & (1ULL << (page_index % PAGES_PER_BITMAP_INDEX));
}

// Gets the index into the paging bitmap.
static inline uint64_t addr_to_page_index(struct PHYSICAL_MEMORY_CONTEXT *memory, phys_addr_t addr)
{
	if (addr % PAGE_BYTE_SIZE != 0)
	{
		// Address is not aligned.
		printf(KERROR "Cannot convert address to page index. Address is not aligned.\n");
		return INVALID_PHYS;
	}
...
static inline void reserve_page(struct PHYSICAL_MEMORY_CONTEXT *memory, uintptr_t page_index)
{
	memory->bitmap[page_index / PAGES_PER_BITMAP_INDEX] |= (1ULL << (page_index % PAGES_PER_BITMAP_INDEX));
	memory->used_pages++;
}

// Sets a single page frame as available
static inline void release_page(struct PHYSICAL_MEMORY_CONTEXT *memory, uintptr_t page_index)
{
	memory->bitmap[page_index / PAGES_PER_BITMAP_INDEX] &= ~(1ULL << (page_index % PAGES_PER_BITMAP_INDEX));
	memory->used_pages--;
}
...
static bool can_store_pages(uintptr_t page_index, size_t size_in_pages, size_t total_pages)
{
	for (uintptr_t i = page_index; i < page_index + size_in_pages; i++)
	{
		if (i >= total_pages || is_page_used(&_pm_context, page_index)) // TODO pass in
		{
			return false;
		}
	}
...
static uintptr_t find_pages(size_t pages_needed, uintptr_t start_page, uintptr_t end_page)
{
...
struct MEMORY_BITMAP init_physical_memory(void)
{
	...
	_pm_context.bitmap = (uint64_t *)(suitable_bitmap_entry->base + hhdm_request.response->offset);
