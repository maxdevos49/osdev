...
const char *debug_code_str(err_code code)
{
	...
	case ERROR_ADDRESS_ALIGNMENT:
		return STRINGIFY(ERROR_ADDRESS_ALIGNMENT);
	case ERROR_INVALID_ADDRESS:
		return STRINGIFY(ERROR_INVALID_ADDRESS);
	case ERROR_INSUFFICIENT_SPACE:
		return STRINGIFY(ERROR_INSUFFICIENT_SPACE);
	case ERROR_NOT_FOUND:
		return STRINGIFY(ERROR_NOT_FOUND);
	}
